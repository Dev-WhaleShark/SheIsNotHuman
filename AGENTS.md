...
- 사용자가 위임한 작업 난이도별 모델 선택을 적용하며, 일반적인 모델·추론 수준 선택에 다시 허락을 구하지 않는다. 탐색, 문서, 간단한 구현은 `gpt-6-luna` / `high`, 일반 구현과 감독은 `gpt-6-sol` / `medium`, 복잡한 버그 분석과 독립 검증은 `gpt-6-sol` / `high`를 기본으로 한다. 가장 어려운 추론이 필요한 경우에만 근거를 작업 기록에 남기고 `gpt-6-astra` / `low`로 올린다. 실제 런타임이 요청한 모델을 지원하지 않으면 확인된 사용 가능 모델 중 가장 가까운 낮은 등급으로 대체하고 요청값, 실제값, 이유를 기록한다.
...
- 최상위 추론으로 올리는 경우: 문제 증거와 이전 모델로 부족한 이유를 기록한 뒤 `gpt-6-astra` / `low`를 요청한다.
