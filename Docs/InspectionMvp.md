...
2026-09-23: 서류 펼쳐서 검사, 방문자 보기, 책상 보기 보조 버튼과 전용 연결 코드를 제거했습니다. 서류를 직접 클릭해 검사하고, 시점은 WASD·방향키·가장자리 버튼으로 전환합니다.
...
## 문서 원본 서식 편집 도구 (2026-09-23)

`InspectionDocumentLayout.Apply()` / `Tools > Inspection MVP > Apply document layout`은 현재 MVP 씬의 Edit Mode에서 실제 `IdentityDocument.prefab`, `OrderDocument.prefab`을 갱신한다. 기존 제목 TMP와 초상화, 루트 Button·DeskInspectableItem 및 프리팹 GUID를 보존하며 이름으로 자식을 재사용한다. 기존 Expanded 프리팹은 수정하지 않는다. 프리팹은 저장하고 씬은 dirty로 남기므로 적용 결과를 확인한 뒤 씬을 저장한다.

- 신분증: 460×280 가로 카드, 회녹색 종이와 얇은 테두리, 왼쪽 이름·고객 코드, 오른쪽 초상화, 하단 실제 NPC 식별자. 책상에서는 균일 배율 0.68, 위치 (-230, -20).
- 주문서: 310×480 세로 휴대전화 형태, 어두운 테두리와 연녹색 화면, 주문자 이름·고객 코드 칸, 주문 번호·상품·수량 칸. 책상에서는 균일 배율 0.55, 위치 (210, -20).
- 정적 라벨과 실제 데이터 TMP를 분리하고 모든 문서 텍스트의 raycast를 끈다. 날짜·금액·가림 문자열을 임의로 만들지 않는다. 기존 한국어 폰트를 재사용하며 정적 라벨은 확인된 글자로 구성한다.

이 절은 구현 설명이며 새 서식의 Unity 컴파일·실제 화면·확대/복귀 검증 결과를 뜻하지 않는다. 그 검사는 통합된 런타임 코드와 프리팹에 대해 별도로 수행한다.
